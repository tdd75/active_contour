...
def visual_callback_2d(background, fig=None):
    ...
    def callback(levelset):
        ...
        plt.savefig(
            "contour_images/contour_{0}.png".format(args["image"].split(".")[0].split("/")[-1]))
...
IMG_SIZE = (args['resize'][0], args['resize'][1])
...
while True:
    ...
    # if the 'c' key is pressed, start drawing
        ...
        # surf_key_points, surf_descriptors = surf.detectAndCompute(
        #     cv2.cvtColor(cropped_image, cv2.COLOR_BGR2GRAY), mask=mask / 255)

        # # print("{0} : {1}".format(i, len(surf_key_points)))

        # img_with_keypoints = cv2.drawKeypoints(
        #     cropped_image, surf_key_points, None, (255, 0, 0), 4)

        # cv2.imshow("SURF Keypoints", img_with_keypoints)
        # cv2.imwrite("keypoints/surf_keypoints{0}.png".format(
        #     args["image"].split(".")[0].split("/")[-1]), img_with_keypoints)
        ...
        # sift_img_with_keypoints = cv2.drawKeypoints(
        #     cropped_image, sift_keypoints, None, (255, 0, 0), 4)

        # cv2.imshow("SIFT Keypoints", sift_img_with_keypoints)
        # cv2.imwrite("keypoints/sift_keypoints{0}.png".format(
        #     args["image"].split(".")[0].split("/")[-1]), sift_img_with_keypoints)
